            ...
            <Column :header="$t('actions')" headerClass="[&>div]:justify-end [&>div]:pl-5 w-20">
                <template #body="{ data }">
                    <div v-if="data.status != 'done'" class="flex justify-end">
                        ...
                        <Button v-else icon="pi pi-check" class="ml-6" rounded text severity="success"
                            :loading="data.doing" @click="done(data)" />
                    </div>
                </template>
            </Column>
        </DataTable>
...
import TreatmentPlanDetails from '../components/TreatmentPlanDetails.vue';
import TreatmentPlanInstallments from '../components/TreatmentPlanInstallments.vue';
